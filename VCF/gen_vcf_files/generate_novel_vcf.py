...
OUTPUT_DIR = os.path.join(PROJECT_DIR, "VCF", "Generated vcf files")
...
def _simulate_sift(grantham_score: int, rng: np.random.Generator) -> str:
    ...
    return "Damaging" if rng.random() < prob_deleterious else "Tolerated"
...
def _simulate_pp2(grantham_score: int, rng: np.random.Generator) -> str:
    ...
    if grantham_score < 60:
        return rng.choice(["B", "P"])
    elif grantham_score < 120:
        return rng.choice(["P", "D"])
    else:
        return "D"
...
if __name__ == "__main__":
    ...
    generate_novel_vcf(
        os.path.join(OUTPUT_DIR, "novel_germline.vcf"),
        n_variants=50, profile="germline", seed=42,
    )
    generate_novel_vcf(
        os.path.join(OUTPUT_DIR, "novel_somatic.vcf"),
        n_variants=50, profile="somatic", seed=77,
    )

    # Patient VCF: mix of germline + somatic
    print("\n  Generating mixed patient VCF...")
    germ_path = os.path.join(OUTPUT_DIR, "_tmp_novel_g.vcf")
    soma_path = os.path.join(OUTPUT_DIR, "_tmp_novel_s.vcf")
    ...
    patient_path = os.path.join(OUTPUT_DIR, "novel_patient.vcf")
