...
# 1. Load the dataset
data = pd.read_csv('../../IARC_TP53_DB/GermlineDownload_r21.csv')

#count and calculate percentage
grouped_topography = data['Short_topo'].value_counts(ascending=True)/len(data)*100
...
plt.tight_layout()
plt.savefig('germline_cancer_occurrence_plot.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('germline_mutation_effect_pie.png', dpi=300, bbox_inches='tight')
...
plt.text(0, 5.5, f'while remaining {hist[0]/len(codon_number)*100:.2f} % are intron mutations'
         , fontsize=10)
plt.savefig('germline_codon_number_distribution.png', dpi=300, bbox_inches='tight')
plt.show()